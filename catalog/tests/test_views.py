...
class BorrowBookViewTest(TestCase):
    ...
    def test_borrow_book_view_post_request(self):
        self.client.force_login(self.user)
        form_data = {
            'bookcopy_id': self.book_copy.id,
            'start_date': date.today(),
            'due_date': date.today()
        }

        response = self.client.post(self.url, data=form_data)
        self.assertRedirects(response, reverse('book-detail', args=(self.book.id,)))
        borrowing = Borrowing.objects.first()
        self.assertEqual(borrowing.book_copy, self.book_copy)

class CancelBorrowingViewTest(TestCase):
    @classmethod
    def setUp(self):
        self.user = User.objects.create_user(username='testuser', password='testpassword')
        self.book = Book.objects.create(
    ...
    def test_cancel_borrowing(self):
        ...
        self.assertEqual(self.borrowing.status, 'c')

    def test_cancel_borrowing_invalid_id(self):
        invalid_url = reverse('borrowing-cancel', args=(9999,))
